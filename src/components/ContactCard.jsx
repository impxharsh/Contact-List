...
const FallbackAvatar = ({ name }) => {
  ...
  return (
    <div
      ...
      // Removed rounded-t-lg so it fills the space
      ...
      <div className="relative w-full h-48 bg-gray-200 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700">
        {!hasError && imgSrc ? (
          <>
            ...
            <img
              src={imgSrc}
              alt="" // Decorative, so alt tag is empty
              className="absolute inset-0 w-full h-full object-cover filter blur-lg scale-110" // blur-lg and scale-110 to fill edges
              referrerPolicy="no-referrer"
            />
            {/* 2. The "Real" Image on top, not cropped */}
            <img
              src={imgSrc}
              alt={`${name}'s profile`}
